...
async def show_student_course_menu(callback: CallbackQuery, state: FSMContext):
    ...
    logger.info(f"User {callback.from_user.id} selected student course {course_id}. Displaying course menu. Language: {lang}.")
...
async def show_student_schedule(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    logger.info(f"User {callback.from_user.id} viewed schedule for course {course_id}. Language: {lang}.")
...
async def show_student_materials(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    logger.info(f"User {callback.from_user.id} viewed materials for course {course_id}. Language: {lang}.")
...
async def return_to_student_courses_list(
    ...
    await callback.answer()
    logger.info(f"User {callback.from_user.id} returned to student courses list. Language: {lang}.")
